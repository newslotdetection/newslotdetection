...
class PretrainKCLManager:
    ...
    def train(self, args, data):  
        ...
        best_model = None
        best_eval_score = 0
        for epoch in trange(int(args.num_pretrain_epochs), desc="KCL-Epoch"):

            self.model.train()
            tr_loss = 0
            nb_tr_examples, nb_tr_steps = 0, 0
            
            for step, batch in enumerate(tqdm(self.train_dataloader, desc="KCL-Iteration")):
                ...
                loss = self.model(input_ids, segment_ids, input_mask, train_target, loss_fct = self.loss_fct, mode = 'train', bin_label_ids=bin_label_ids)
                loss.backward()
    ...
    def get_outputs(self, args, mode = 'eval'):
        ...
        for batch in tqdm(dataloader, desc="KCL-Iteration"):
            ...
            with torch.set_grad_enabled(False):
                ...
                features, logits = self.model(input_ids, segment_ids, input_mask, mode = 'eval', bin_label_ids=bin_label_ids)
