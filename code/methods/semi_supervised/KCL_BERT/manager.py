...
class KCLManager:
    ...
    def __init__(self, args, data, model, logger_name = 'Discovery'):
        ...
        self.Data = data
        #print([self.tokenizer.ids_to_tokens[i] for i in [101,  7632,  1012,  1045,  1005,  1049,  2667,  2000,  2424,  1037]])
        ...
        pretrain_manager = PretrainKCLManager(args, data, model)   #args.backbone = 'bert_KCL_simi'
        ...
        if args.train:
            ...
            self.logger.info('num_labels: %s', data.num_labels)
    ...
    def load_pretrained_model(self, pretrained_model):
        ...
        classifier_params = ['classifier.weight','classifier.bias', 'dense']
    ...
    def train(self, args, data): 
        ...
        best_eval_score = 0
        eval_score_is_0 = 0
        for epoch in trange(int(args.num_train_epochs), desc="KCL-train-Epoch"):  
            
            tr_loss = 0
            nb_tr_examples, nb_tr_steps = 0, 0
            self.model.train()

            for batch in tqdm(self.train_dataloader, desc="KCL-train-Iteration"):
                ...
                loss = self.model(input_ids, segment_ids, input_mask, label_ids, mode = 'train', simi = simi, loss_fct = self.loss_fct, bin_label_ids = bin_label_ids)
    ...
    def get_outputs(self, args, mode = 'eval', get_feats = False):
        ...
        for batch in tqdm(dataloader, desc="KCL-KCL-Iteration"):
    ...
    def prepare_task_target(self, batch, model):
        ...
        input_ids, input_mask, segment_ids, bin_label_ids, slot_label_ids, label_ids = batch
        _, logits = model(input_ids, segment_ids, input_mask, bin_label_ids=bin_label_ids)
    ...
    def get_results_samp(self, args, dataloader, mode='gt', data_type='unlabel', write_result=True):
        ...
        for batch in tqdm(dataloader, desc="KCL-Iteration of prediction"):
            ...
            if mode=='hyp':
                ...
                total_probs = F.softmax(total_logits.detach(), dim=1)
                total_maxprobs, total_preds = total_probs.max(dim = 1)
                #total_maxprobs2, total_preds2 = total_logits.max(dim = 1)
