...
class PretrainDeepAlignedManager:
    ...
    def train(self, args, data):
        ...
        for epoch in trange(int(args.num_pretrain_epochs), desc="DeepalignEpoch"):
            
            self.model.train()
            tr_loss = 0
            nb_tr_examples, nb_tr_steps = 0, 0
            
            for step, batch in enumerate(tqdm(self.train_dataloader, desc="DeepalignIteration")):
                batch = tuple(t.to(self.device) for t in batch)
                #input_ids, input_mask, segment_ids, label_ids = batch
                input_ids, input_mask, segment_ids, bin_label_ids, slot_label_ids, label_ids = batch
                with torch.set_grad_enabled(True):

                    loss = self.model(input_ids, segment_ids, input_mask, label_ids, mode = "train", loss_fct = self.loss_fct, bin_label_ids = bin_label_ids)
    ...
    def get_outputs(self, args, mode = 'eval', get_feats = False):
        ...
        for batch in tqdm(dataloader, desc="DeepalignIteration"):
